...
// `owner` = the doc id that DECLARED this edge, so an incremental rebuild can
// remove exactly one doc's edges. A logical edge can be declared by two docs
// (SUPERSEDES from both endpoints); last writer wins the row + owner, and the
// incremental builder restores any cross-owned SUPERSEDES edge whose other
...
const CREATE_GRAPH_SOURCE_FILES = `
...
const CREATE_EDGES_OWNER_IDX = 'CREATE INDEX IF NOT EXISTS idx_edges_owner ON edges(owner);';
